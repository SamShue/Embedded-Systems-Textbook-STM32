...
#### Additional Example: Swapping values in memory with pointers

This example highlights what is happening in memory during the function call. The pointer arguments do not pass copies of the values; instead, they pass the addresses of the original variables. The function stores one of the values in a temporary local variable, writes the new value to the first address, and then writes the saved value to the second address.
...
Before the call, the stack contains the variables `a` and `b` in `main`. During the function call, the addresses of those variables are passed to `swap`, which creates its own temporary value, `temp`, on the stack. After the assignments complete, the original memory locations for `a` and `b` have been updated, even though the variables themselves are still defined in `main`.

![Stack frame layout showing how a function call stores local variables and parameters in memory](../images/ch02/stack_foo_frame.png){#fig-ch02-pointer-swap-stack-frame}

![The updated stack state after the swap call returns to `main`](../images/ch02/stack_main_frame.png){#fig-ch02-pointer-swap-after-return}
